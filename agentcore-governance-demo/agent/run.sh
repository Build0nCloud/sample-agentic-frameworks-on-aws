...
# ── Run ──────────────────────────────────────────────────────
# Headless (one-shot): --dangerously-skip-permissions is safe here because
#   --print mode never shows the interactive bypass-permission warning screen.
# Interactive: do NOT use --dangerously-skip-permissions (it forces a one-time
#   warning screen). Instead, gateway MCP tools are pre-approved via the
#   permissions.allow list in settings.json, so no prompts appear.
if [ $# -gt 0 ]; then
